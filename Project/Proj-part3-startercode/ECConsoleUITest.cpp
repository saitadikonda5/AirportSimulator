...
//#if 0       
    ...
    std::ofstream outputFile("list_box_texts.txt");

    // Check if the file stream is successfully opened
    if (outputFile.is_open()) {
        // Write the text from the list box to the file
        for (int i = 0; i < 6; ++i) {
            std::string text = pl->GetTextAtRow(i);
            outputFile << "Text at row " << i << ": " << text << std::endl;
        }

        // Close the file stream
        outputFile.close();
    } else {
        // If the file stream cannot be opened, print an error message
    ...
    ECConsoleUITextLabel *pl9 = new ECConsoleUITextLabel(15, 25, "-----------------");
    viewTest.AddUIItem(pl9);
    //ECConsoleUIButton *pb1 = new ECConsoleUIButton(15, 26, 25, 26, "Register!");
    //viewTest.AddUIItem(pb1);
    ECConsoleUITextLabel *pl10 = new ECConsoleUITextLabel(15, 27, "-----------------");
