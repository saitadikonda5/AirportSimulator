...
{
public:
    friend class ECConsoleUITextField; // Allow ECConsoleUITextField to access private members
    ECTextViewImp();
    
    void StartEditing() { isEditing = true; } // Method to start editing
    void StopEditing() { isEditing = false; } // Method to stop editing
    
    bool IsEditing() const { return isEditing; } // Method to check editing state
    ...
    // Note: you must clear all colors if you want to undo coloring
...
private:
    ...
    bool isEditing; // Flag to track editing state
